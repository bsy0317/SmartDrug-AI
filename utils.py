...
def background_subtraction(img, mask):
    ...
    img_output = cv2.bitwise_and(img, mask)
    return img_output
...
def pill_mask(img_root, mask_root, pre_root):
    ''' Mask Pill Mask
    ...
    for i, img_name in enumerate(tqdm(img_list, desc="pill_mask", mininterval=0.1)):
        #print('Processing %d/%s' % (i+1, img_name))
        img = cv2.imread(img_name)
        shear = int(img.shape[0] * 0.12)
        pad_img = np.zeros((shear, img.shape[1]))
        img = img[:(img.shape[0]-shear), :, :]
        backup_image = img[:1].copy()

        # Gaussian Smoothing Filter
        img = (gaussian(img, sigma=2, channel_axis=True)*255).astype(np.uint8)

        # SLIC (Simple Linear Iterative Clustering)
        labels = slic(img, n_segments=150, compactness=12, max_num_iter=10)
        labels = labels + 1
        ...
        cv2.imwrite(os.path.join(mask_root, os.path.basename(img_name)), labels)
        clr_bg = background_subtraction(cv2.imread(img_name), cv2.imread(os.path.join(mask_root, os.path.basename(img_name))))
        cv2.imwrite(os.path.join(pre_root, os.path.basename(img_name)), clr_bg)
...
#    pill_mask('./data/image','./data/mask','./data/rmbg')